...
const app = useLocalizedApp(() => product.value!)

useSeoMeta({
  title: () => app.value.name,
  description: () => app.value.seo.description,
  ogTitle: () => `${app.value.name} | MiniDev`,
  ...
  <div>
    <section class="site-hero-bg relative overflow-hidden pt-24 pb-16 sm:pt-28 lg:pt-36 lg:pb-24">
      <div class="site-container">
        <div class="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            <span :class="['grid size-16 place-items-center rounded-2xl bg-gradient-to-br text-white shadow-xl', app.gradient]"><UIcon :name="app.icon" class="size-9" /></span>
            <p class="eyebrow mt-6">{{ app.category }}</p>
            <h1 class="display-title mt-3">{{ app.name }}</h1>
            <p class="body-lead mt-5">{{ app.description }}</p>
            <div class="mt-8 flex flex-col gap-3 sm:flex-row"><UButton size="xl" class="brand-action" @click="open('trial', app.slug)">{{ t('product.tryFree', { name: app.name }) }}</UButton><UButton to="/pricing" size="xl" class="brand-action">{{ t('product.viewPricing') }}</UButton></div>
            <p class="mt-4 text-sm text-zinc-500">{{ t('product.demoNote') }}</p>
          </div>
          <ProductMockup :title="`MiniDev ${app.name}`" demo :video="app.video" :poster="app.poster" />
        </div>
      </div>
    </section>
    ...
    <section class="section-space">
      <div class="site-container space-y-16 lg:space-y-24">
        <article v-for="(benefit, index) in app.benefits" :key="benefit.title" class="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
    ...
    <section class="section-space">
      <div class="site-container">
        ...
        <div class="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          <article v-for="feature in app.features" :key="feature.title" class="rounded-2xl bg-white p-6 ring-1 ring-zinc-200 dark:bg-zinc-950 dark:ring-zinc-800">
    ...
    <section class="section-space bg-brand-50 dark:bg-brand-950/30">
      <div class="site-container grid gap-10 lg:grid-cols-[0.7fr_1.3fr]">
        <div><p class="eyebrow">{{ t('product.questions') }}</p><h2 class="section-title mt-3">{{ t('product.about', { name: app.name }) }}</h2></div>
        <UAccordion :items="app.faqs" :ui="{ item: 'border-b border-brand-200 dark:border-brand-800' }" />
      </div>
    </section>
  </div>
</template>
